...
const { bufferToBase64url, getExpectedOrigins, getExpectedRpIds } = require('../../api/_lib/webauthn');
const { createSecuredHandler, addAuditEntry, sanitizeString } = require('../../api/_lib/security');
const {
    CONFIG,
    generateChallenge,
    storeChallenge,
...
} = require('../../api/_lib/passkey-utils');
...
const passkeyRegisterHandler = async (req, res, { clientIp, validatedBody }) => {
    try {
        ...
        if (action === 'list') {
            ...
            const { verifySessionToken } = require('../../api/_lib/passkey-utils');
        ...
        if (action === 'delete') {
            ...
            const { verifySessionToken, deleteCredential } = require('../../api/_lib/passkey-utils');
            const payload = verifySessionToken(sessionToken);
