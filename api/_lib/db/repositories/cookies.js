...
async function updatePreferences(idType, id, updates, metadata = {}) {
    ...
    const current =
        idType === 'user' ? await getPreferencesForUser(id) : await getPreferencesByCookieId(id);
...
async function getEnabledCookies(cookieIdOrUserId, isUser = false) {
    ...
    for (const cookie of Object.values(GranularCookies)) {
        enabled.granular[cookie] = await isGranularCookieEnabled(cookieIdOrUserId, cookie, isUser);
    }
