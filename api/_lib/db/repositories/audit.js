...
async function getLogsByDate(date, options = {}) {
    ...
    const params = {
        KeyConditionExpression: startTime ? 'PK = :pk AND SK BETWEEN :start AND :end' : 'PK = :pk',
...
async function generateAuditSummary(startDate, endDate) {
    ...
    summary.successRate =
        summary.totalEvents > 0 ? Math.round((successCount / summary.totalEvents) * 100) : 100;
...
async function exportAuditLogs(options = {}) {
    const { startDate, endDate, userId, actions, format = 'json' } = options;
