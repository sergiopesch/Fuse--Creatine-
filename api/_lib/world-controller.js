...
const CREDIT_THRESHOLDS = {
  ...
  CRITICAL: 0.90,     // 90% - Red critical
  HARD_STOP: 1.00     // 100% - Auto-pause everything
};
...
let worldState = {
  ...
  creditProtection: {
    ...
    monthlyLimit: 500.00,        // $500 monthly limit
    currentDailySpend: 0,
...
function getPendingActions() {
  return {
    ...
    totalEstimatedCost: worldState.pendingActions.reduce((sum, a) => sum + (a.estimatedCost || 0), 0)
  };
}
...
function checkCreditLimits() {
  ...
  } else if (dailyUsageRatio >= CREDIT_THRESHOLDS.CRITICAL || monthlyUsageRatio >= CREDIT_THRESHOLDS.CRITICAL) {
    status = 'critical';
    message = 'Critical credit usage - approaching limit';
  } else if (dailyUsageRatio >= CREDIT_THRESHOLDS.CAUTION || monthlyUsageRatio >= CREDIT_THRESHOLDS.CAUTION) {
    status = 'caution';
    message = 'High credit usage - use caution';
  } else if (dailyUsageRatio >= CREDIT_THRESHOLDS.WARNING || monthlyUsageRatio >= CREDIT_THRESHOLDS.WARNING) {
    status = 'warning';
...
module.exports = {
  ...
  getState: () => worldState,
  setState: (newState) => { worldState = { ...worldState, ...newState }; }
};
